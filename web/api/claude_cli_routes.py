...
def _build_system_prompt(client, body: 'ChatRequest', attached_paths: list[str],
    ...
    parts = [
        ...
        '',
        '🔹 **Локальный** (cwd: Read/Glob/Grep) — только файлы, к которым у пользователя уже ЕСТЬ ДОСТУП. Это маленькое подмножество всего серверного кода.',
        '🔹 **Глобальный** (`mcp__cp-ai__search_code`) — поиск по ВСЕМУ коду игрового сервера через платформу. Включает файлы, которых нет у пользователя локально.',
        '',
        'Когда пользователь говорит «найди» / «есть ли» / «где вызывается» / «где упоминается» — он имеет в виду ПОИСК ПО ВСЕМУ КОДУ, а не только по тому что есть локально. Используй `mcp__cp-ai__search_code`.',
        'Локальный Grep подходит когда явно работаете с конкретным открытым файлом (есть прикреплённые @-файлы или пользователь явно сказал «в этом файле»).',
        'Если локальный Grep что-то выдал — это НЕ значит что ты нашёл всё. Через `search_code` может быть гораздо больше совпадений в других файлах.',
        '',
        '## Формат вывода результатов поиска',
        'Когда показываешь результат `search_code` — выводи ВСЕ найденные файлы и блоки (не выбирай "ключевые места" — пользователь сам решит что важно).',
        'Формат — markdown-таблица с колонками: **Файл**, **Блок**, **Строки**. Пример:',
        '',
        '| Файл | Блок | Строки |',
        '|------|------|--------|',
        '| `gamemodes/gamelogic.pwn` | `StartPlayerJob` | 44766 |',
        '| `include/system/jobs/trains/railway.pwn` | `StartMotorman` | 296 |',
        '',
        'Не превращай таблицу в комментарий-список. Используй именно таблицу.',
        '',
        '# Когда что-то менять',
        ...
        '',
        '# Как работать',
        '- Поиск "где X" / "есть ли X" / "найди X" → СНАЧАЛА `mcp__cp-ai__search_code` (по всему серверу). Локальный Grep — только когда явно работаешь с открытым файлом.',
        '- Изучай код через Glob → Grep → Read (с offset/limit) — это для локальных файлов.',
        ...
        '',
        '1. ВСЕГДА сначала вызови `task_list_brief` — он показывает ВСЕ задачи И ВСЕ кейсы внутри них. Прочитай ВНИМАТЕЛЬНО прежде чем добавлять.',
        ...
        '   - Если задача с таким title уже есть в task_list_brief — используй её id или повтори тот же task_title, кейс добавится туда.',
        '   - Иначе задача создастся автоматически при первом `task_add_case` с этим task_title.',
        '4. Для каждого пункта списка СНАЧАЛА проверь нет ли уже похожего кейса в выводе task_list_brief.',
        '   - Сравнивай по смыслу, не только по словам. "Доработать уведомление" и "Уведомление дополнить описание" — это ОДИН кейс.',
        '   - Если похожий есть — НЕ добавляй. Просто пропусти и упомяни в финальном резюме что пропустил.',
        '5. Если кейс новый — вызови `task_add_case`. case_title — короткая суть (1 строка без префикса), description — оригинальный текст пункта без сокращений.',
        '6. Для каждого нового кейса заполни `ai_analysis` (1-3 предложения: что это значит, какой риск, что важно учесть) и `ai_proposal` (1-3 предложения: как ты предлагаешь это решить — БЕЗ КОДА, только подход).',
        '7. НЕ генерируй сами правки кода в этом шаге. Код будет генерироваться позже, по запросу пользователя на конкретный кейс.',
        '8. Когда закончил — напиши краткое резюме: "Создал N кейсов в M задачах. Пропустил K дубликатов: [список]". Без markdown-таблицы.',
        '',
    ...
    # Задача из контекста треда. main — это "без привязки", туда задача
    # НЕ подмешивается даже если есть глобально активная (иначе в основном
    # чате будут торчать задачи которыми пользователь занимается в других
    # тредах).
    active = None
    try:
        ...
        if thread_id and thread_id != ai_threads_store.MAIN_ID:
            ...
            # Для тредов задач, не привязанных явно, всё равно НЕ берём
            # глобальную — у тебя для этого свой тред.
    except Exception:
        active = None
    if active:
        parts.append('')
